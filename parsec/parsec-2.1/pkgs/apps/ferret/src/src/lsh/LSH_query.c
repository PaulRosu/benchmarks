...
static void LSH_query_local (LSH_query_t *query)
{
	...
	for (j = 0; j < K-1; j++)
	{
		if (query->topk[K - j - 2].dist >= ((float)3.40282346638528860e+38)) break;//HUGE
		lk = log(j+1); 
...
static void LSH_query_bootstrap (LSH_query_t *query, const float *point)
{
	...
	for (i = 0; i < L; i++)
	{
		...
		TOPK_INIT(_topk[i], dist, K, ((float)3.40282346638528860e+38));//HUGE
...
void LSH_query_merge (LSH_query_t *query)
{
	...
	TOPK_INIT(topk, dist, K, ((float)3.40282346638528860e+38));//HUGE
